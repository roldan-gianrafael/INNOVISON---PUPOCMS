    ...
    public function reports()
{
    ...
    public function notificationHistory()
    {
        ...
        $notifications = $this->buildAdminNotifications($user, false, 40);
    ...
    private function buildAdminNotifications(User $user, bool $unreadOnly = true, int $limit = 10)
    {
        $isStudentAssistant = $this->isStudentAssistantAccount($user);
        ...
        $todayApprovedAppointments = Appointment::query()
        ...
        foreach ($recentPendingAppointments as $appointment) {
            $notifications->push([
                ...
                'chips' => ['Pending'],
                'occurred_at' => optional($appointment->updated_at ?? $appointment->created_at)->toIso8601String(),
            ]);
        }
        ...
        foreach ($recentHealthFormSubmissions as $healthProfile) {
            ...
            $notifications->push([
                ...
                'chips' => ['Pending review'],
                'occurred_at' => optional($healthProfile->updated_at ?? $healthProfile->created_at)->toIso8601String(),
            ]);
        }
        ...
        foreach ($medicineAlerts as $medicineAlert) {
            ...
            $notifications->push([
                ...
                'chips' => [$isExpired ? 'Expired' : 'Near expiry'],
                'occurred_at' => optional($medicineAlert->updated_at ?? $medicineAlert->expiration_date)->toIso8601String(),
            ]);
        }

        $notifications = $notifications
            ->map(function (array $notification) use ($readMap) {
                $notification['is_unread'] = !isset($readMap[$notification['id']]);
                return $notification;
            })
            ->sortByDesc(function (array $notification) {
                return strtotime((string) ($notification['occurred_at'] ?? now()->toIso8601String()));
            });

        if ($unreadOnly) {
            $notifications = $notifications->filter(fn (array $notification) => $notification['is_unread']);
        }

        return $notifications->take($limit)->values();
    }
